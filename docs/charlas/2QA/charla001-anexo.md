...
6. **Los tres pendientes, implementados el mismo día, con un tercer verificador independiente.** pySigHor-Oficina aplicó los tres puntos de la auditoría en [PR #477](https://github.com/mmasias/pyCelda/pull/477) (commit [`5f8ee94`](https://github.com/mmasias/pyCelda/commit/5f8ee94f813ea63a1dafaad437d3b5e99bcf0ac0)), verificando primero las cifras del informe contra el repo real antes de darle crédito -- mismo criterio que pySigHor y la propia auditoría ya habían aplicado entre sí. La auditoría, a su vez, no aceptó "aplicado y desplegado" como afirmación: comprobó el diff real de los 4 ficheros, el estado en vivo de [GitHub Actions](https://github.com/mmasias/pyCelda/actions/runs/35924703723) (ambos jobs en verde), y las cabeceras de seguridad *ya sirviendo en producción real* con un `curl -I` directo contra el dominio público -- [respuesta publicada aquí](https://github.com/mmasias/pyCelda/discussions/475#discussioncomment-18572914).

Ninguno de los cuatro pasos (2, 4, 5, 6) confió en la palabra del paso anterior. Cada uno verificó contra el sistema real antes de aceptar o corregir.
...
**El propio CI, en su primer minuto de vida, encontró lo que dos rondas de revisión humana/agéntica no habían anticipado.** Al implementar los tres pendientes, aparecieron dos problemas reales que ni el informe de auditoría ni la primera verificación de pySigHor habían previsto: `httpx` 0.28 rompe `TestClient` (fijado a `<0.28` en `pyproject.toml`), y `uv pip install --system` falla en `ubuntu-latest` por PEP 668 (Python "externally managed"), corregido a `uv run --extra dev pytest -q`. Es el argumento de la propia charla -- prevenir cuesta menos que corregir -- funcionando al revés de lo esperado: aquí fue la infraestructura de prevención (el CI) la que reveló, nada más nacer, un defecto que el proceso de revisión humana llevaba dos rondas sin ver.
...
- Comentario de pySigHor-Oficina aplicando los tres pendientes: <https://github.com/mmasias/pyCelda/discussions/475#discussioncomment-18572893>
- Verificación final, independiente, de la implementación real y de producción: <https://github.com/mmasias/pyCelda/discussions/475#discussioncomment-18572914>
...
- PR de implementación de los tres pendientes: <https://github.com/mmasias/pyCelda/pull/477>, commit de merge <https://github.com/mmasias/pyCelda/commit/5f8ee94f813ea63a1dafaad437d3b5e99bcf0ac0>
- Ejecución real de CI en verde: <https://github.com/mmasias/pyCelda/actions/runs/35924703723>
